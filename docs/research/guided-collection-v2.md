...
현재 수집기는 Lite 관절 추정기를 사용한다. 이는 자세·활동 분류 모델을 새로 학습한 것이 아니다. 실제 M1 Air FPS와 관절 정확도는 별도 검증 대상이다. 기존 Heavy CSV와 새 Lite CSV는 `pose_model`로 구분하고, 모델 차이를 검증하기 전에는 같은 분포로 간주하지 않는다.
